...
2 つのデータソースは異なる識別子スキームを使っており、各ツールはどちらかに固定されます：

| ツール                                            | スキーム                                                  | 例                                        |
| ------------------------------------------------- | --------------------------------------------------------- | ----------------------------------------- |
| `compat_check`, `compat_compare`, `compat_search` | **BCD ドット表記** — 細粒度、API は camelCase             | `api.PushManager`, `css.properties.grid`  |
| `compat_get_baseline`, `compat_list_baseline`     | **web-features ケバブケース** — 粗粒度、機能グループ単位  | `push`, `container-queries`               |
| `compat_check_support`                            | ブラウザ ID + バージョン文字列                            | `safari` + `17`                           |

BCD は細粒度（`api.PushManager`, `api.PushEvent`, `api.PushSubscription` が別エントリ）、web-features はまとめた粒度（`push` 一つで上の 3 つをカバー）になっています。識別子が不明な時は `compat_search` を最初に叩くのが確実です。入力正規化により、ケバブケース（`view-transition` → `viewtransition`）や末尾 `.0` 付きバージョン（`17.0` → `17`）は自動でフォールバック検索されます。
...
> 「PWA で Push API を使いたいが、今実現できる？」

```
Step 1 → compat_check        feature: "api.PushManager"
         # API単位のブラウザ対応状況（Chrome 42+, Safari 16+, Firefox 44+）

Step 2 → compat_get_baseline  feature: "push"
         # 機能グループ単位の視点：2023-03-27 から Newly Available

Step 3 → compat_compare       features: ["api.PushManager", "api.Notification"]
         # PWA が両方に依存する場合の横並び比較
```

**W3C MCP** や **RFCXML MCP** のような仕様系サーバーと組み合わせれば、「仕様はどう要求している？」と「実際のブラウザで動くのか？」の両方を 1 会話で解決できます。
